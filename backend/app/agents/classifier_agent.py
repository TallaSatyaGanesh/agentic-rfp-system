...
def _determine_category(text: str, section: str) -> tuple[str, str]:
    ...
    category_rules = [
        ...
        (
            r'\b(?:pricing|cost|costs|fee|fees|commercial\s+terms|payment\s+schedule|invoic[a-z]*|payment\s+terms|milestone\s+payment|budget|discount|hourly\s+rate|fixed\s+prices?|rates?\s+card|expenses|currency|financial\s+proposal|financial\s+quote|subscription\s+fee|pro[- ]\s*rata|no\s+(?:extra|additional)\s+cost|free\s+of\s+(?:cost|charge)|at\s+no\s+(?:extra\s+)?cost|taxes|inr\b|usd\b|crores?|lakhs?|indian\s+rupees?|lump\s*sum\s*(?:contract\s*)?price|without\s+any\s+impact|broken\s+period\s+payment)\b',
            "Commercial",
            "Clause establishes pricing model, commercial fees, invoicing terms, payment schedule, or cost allocation"
        ),
        (
            r'\b(?:(?:technical|system|project|software|api|architecture)\s+documentation|user\s+manuals?|technical\s+manuals?|architecture\s+diagrams?|training\s+materials?|runbooks?|api\s+docs|documented\s+restful|as-built|system\s+guides?|specification\s+documents?|operations\s+manuals?|(?:status|sla|performance|availability|compliance|service|progress|incident|audit|activity|management)\s+reports?|(?:monthly|weekly|daily|regular|quarterly|annual|periodic|fortnightly|bi-?weekly)\s+(?:[a-z]+\s+)?reports?|reports?\s+covering\s+(?:availability|incidents|performance|sla|service|compliance)|submit\s+(?:all\s+)?(?:the\s+)?reports?|access\s+for\s+report\s+viewing|reports?\s+must\s+be\s+made\s+available|issues?\s+and\s+resolution\s+log)\b',
            "Documentation",
            "Clause mandates delivery of technical architecture, user manuals, training materials, recurring SLA/service reports, or API runbooks"
        ),
        (
        ...
        (
            r'\b(?:liability|unlimited\s+liability|indemnif[a-z]*|penalt[a-z]*|liquidated\s+damages|warrant[a-z]*|intellectual\s+property|ip\s+(?:rights|infringement)|governing\s+law|jurisdiction|breach|termination|non[- ]disclosure|nda\b|confidential[a-z]*|in\s+confidence|data\s+shared|performance\s+bank\s+guarantee|performance\s+security|security\s+deposit|performance\s+guarantee|pbg\b|bank\s+guarantee|earnest\s+money\s+deposit|emd\b|dispute\s+resolution|force\s+majeure|exit\s+management|handover\s+(?:of\s+data|to\s+rcs|upon\s+expiry)|terms\s+and\s+conditions|enter\s+into\s+an\s+agreement|signing\s+of\s+contract|execution\s+of\s+(?:an\s+)?agreement|execute\s+the\s+agreement|legally\s+bound|deemed\s+to\s+have\s+complied|deemed\s+to\s+be\s+accepted|signoff\s+by\s+both|agreement\s+with\s+the\s+(?:rcs|client|buyer|authority|department)|default\s+on\s+(?:any\s+of\s+the\s+)?service\s+level|performance\s+improvement\s+plan|root\s+cause\s+analysis\s+for\s+[^\n\r.]+?\s+approval|(?:data|information|records?|patient[- ]related\s+information|customer\s+data)\s+(?:shall|will|must)\s+not\s+be\s+used\s+(?:by\s+the\s+vendor\s+)?for\s+(?:advertising|commercial|marketing|unrelated|unauthorized)|(?:vendor|bidder|contractor)\s+(?:shall|will|must)\s+not\s+use\s+(?:customer|client|patient|user|any)\s+(?:data|information)\s+for|(?:ownership|title|custody)\s+of\s+(?:all\s+)?(?:data|information)|data\s+(?:use|usage)\s+restrictions?|restrictive\s+covenants?)\b',
            "Contractual",
            "Clause governs legal liability, contractual commitments, warranties, SLA penalties, PBG/performance security, agreement execution, restrictive covenants, or indemnification"
        ),
        (
            r'\b(?:training|hands[- ]on\s+training|workshop|timeline|milestone|schedule|weeks?\s+of\s+contract|concluded\s+within|completed\s+within|complete\s+(?:implementation|deployment|rollout|delivery|work|project)\s+within|delivery\s+date|go[- ]live|deployment\s+schedule|implementation\s+timeline|delivery\s+timeline|lead\s+time|shipment|freight|project\s+manager\s+nomination|nominate\s+(?:a\s+)?project\s+manager|key\s+personnel|completion\s+date|phase\s+\d+|rollout|user\s+acceptance\s+testing|uat\s+schedule|uat\s+signoff|project\s+implementation\s+plan|implementation\s+plan\s+and\s+(?:clearly\s+)?spell\s+out|important\s+milestones\s+of\s+project|project\s+development\s+duration|development\s+duration|duration\s+should\s+not\s+exceed|not\s+exceed\s+\d+\s*(?:months?|weeks?|days?)|within\s+\d+\s*(?:weeks?|months?|days?|calendar\s+days?|working\s+days?)\s+(?:from|of|after)\s+(?:contract|award|work\s+order|commencement|signing|project|start|issuance|kick-?off|agreement|effective\s+date|date\s+of|receipt|notification)|throughout\s+the\s+period\s+of\s+(?:development|warranty|o&m)|warranty\s+&\s+o&m\s+period|support\s+throughout\s+the\s+period)\b',
            "Delivery",
            "Clause defines implementation timeline, training delivery, rollout schedule, delivery milestone, or project staffing"
        ),
        (
            r'\b(?:administrative\s+(?:forms?|requirements?|details?|instructions?|information|matters?|procedures?)|authorized\s+signatory|point\s+of\s+contact|primary\s+liaison|executive\s+contact|nodal\s+officer|company\s+registration|duns|ein|tin|gst\b|pan\b|tax\s+clearance|notice\s+address|official\s+address|organizational\s+chart|power\s+of\s+attorney|letter\s+of\s+authorization)\b',
            "Administrative",
