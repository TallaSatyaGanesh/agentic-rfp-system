...
def _determine_category(text: str, section: str) -> tuple[str, str]:
    ...
    category_rules = [
        ...
        (
            r'\b(?:liability|unlimited\s+liability|indemnif[a-z]*|penalt[a-z]*|liquidated\s+damages|warrant[a-z]*|intellectual\s+property|ip\s+(?:rights|infringement)|governing\s+law|jurisdiction|breach|termination|non[- ]disclosure|nda\b|confidential[a-z]*|in\s+confidence|data\s+shared|performance\s+bank\s+guarantee|pbg\b|bank\s+guarantee|dispute\s+resolution|force\s+majeure|exit\s+management|handover\s+(?:of\s+data|to\s+rcs|upon\s+expiry)|terms\s+and\s+conditions|enter\s+into\s+an\s+agreement|signing\s+of\s+contract|execution\s+of\s+(?:an\s+)?agreement|legally\s+bound|deemed\s+to\s+have\s+complied|deemed\s+to\s+be\s+accepted|signoff\s+by\s+both|agreement\s+with\s+the\s+(?:rcs|client|buyer|authority|department)|default\s+on\s+(?:any\s+of\s+the\s+)?service\s+level|performance\s+improvement\s+plan|root\s+cause\s+analysis\s+for\s+[^\n\r.]+?\s+approval)\b',
            "Contractual",
